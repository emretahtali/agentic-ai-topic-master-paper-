...
from .new_topic_tools import NewTopicTools
...
class NewTopicAgent(TopicAgent):
    ...
    def __init__(self):
        self.tools_manager = NewTopicTools()
    ...
    def _get_node(self, agent_state: TopicManagerState) -> dict:
        ...
        system_prompt = self._get_other_system_prompt(current_message.content)
        system_message = SystemMessage(system_prompt)

        agent = create_agent(
            model=self.llm,
            response_format=ProviderStrategy(Agents)
        )

        # return agent.invoke({"messages": [system_message, HumanMessage(content="Follow the instruction above and answer.")]})
        response = agent.invoke({"messages": [system_message, HumanMessage(content="Follow the instruction above and answer.")]})
        # print(response["structured_response"].agent)
        response["selected_agent"] = response["structured_response"].agent
        return response
        ...
        #     # "messages": [agent.invoke({"messages": [system_message, HumanMessage(content="Follow the instruction above and answer.")]})]
        #     "messages": [agent.invoke({"messages": [system_message, HumanMessage(content="Follow the instruction above and answer.")]}).get("messages")]
        #     # "messages": [agent.invoke(system_prompt)]
        # }
    ...
    STRICT OUTPUT
    NEVER respond with a text message. Always call select_agent tool with using one of these values for the "agent" parameter:
    {'\n'.join(self.tools_manager.agent_names.keys())}
    ...
    Then call exactly one agent using select_agent tool as specified in STRICT OUTPUT.
    ...
    def _get_other_system_prompt(self, message: str) -> str:
        return f"""You are part of a medical assistant. Your sole task is **agent routing for a new topic**: based ONLY on the latest user message, choose which specialized agent should handle it.
