...
class BenchmarkTemplate:
    ...
    def __init__(
            self,
            llm_type: Literal["openai", "google"],
            model_name: str,
            api_key: str,
            system_prompt: Any,
            result_schema: Any,
    ...
    def _setup_llm(self):
        if self.llm_type == "openai":
            return ChatOpenAI(model=self.model_name, api_key=self.api_key, base_url=self.endpoint, temperature=0)
        elif self.llm_type == "google":
            return ChatGoogleGenerativeAI(model=self.model_name, google_api_key=self.api_key, temperature=0)
        raise ValueError(f"Incompatible LLM type: {self.llm_type}")
            ...
            try:
                for msg in dialog["messages"]:
                    ...
                    if msg["role"] == "user":
                        ...
                        result = await self.agent.ainvoke({"messages": chat_history + [current_msg]})
